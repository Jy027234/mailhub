"""Collect a redacted A1 OAuth artifact from the MailHub audit ledger.

The command is deliberately an HTTP observation client, not a JSON fixture
importer.  It must run after the real OAuth flow, an intentional callback
replay, a credential refresh and a credential revoke have been exercised.  It
accepts no provider token or authorization code and writes only the bounded
``oauth-consent.json`` artifact returned by the audit-derived collector.
"""
    ...
    *,
    ...
    try:
        ...
        with client_factory(
            ...
            trace_id=f"mailhub-activation:{run_id}:oauth",
            authorization=os.getenv("MAILHUB_ACTIVATION_AUTHORIZATION"),
        ) as client:
            client.readiness()
            oauth_payload = client.oauth_observation(
                connection_id=connection_id,
                provider=provider,
                environment=environment,
                run_id=run_id,
            )
        _write_json(evidence_dir / "oauth-consent.json", oauth_payload)
        preflight_path = evidence_dir / "preflight.json"
        if not preflight_path.exists():
            _write_json(preflight_path, preflight)
        print(f"oauth evidence: {evidence_dir / 'oauth-consent.json'}")
        return 0
    except (ActivationError, ValueError, OSError) as exc:
        code = exc.code if isinstance(exc, ActivationError) else "activation_oauth_local_error"
        print(f"oauth evidence blocked: {code}", file=sys.stderr)
        return 2
