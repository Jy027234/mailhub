"""Collect a redacted negative-access artifact after a real revoke/delete run.

The command observes the owner-scoped MailHub connection and audit ledger. It
does not revoke, delete, retry provider calls, accept tokens, or accept an
operator-supplied JSON/count fixture. The real Host Credential Broker must
record the zero-access proof before this command can pass.
"""
    ...
    *,
    ...
    try:
        ...
        with client_factory(
            ...
            trace_id=f"mailhub-activation:{run_id}:revocation",
            authorization=os.getenv("MAILHUB_ACTIVATION_AUTHORIZATION"),
        ) as client:
            client.readiness()
            revocation_payload = client.revocation_observation(
                connection_id=connection_id,
                provider=provider,
                environment=environment,
                run_id=run_id,
            )
        _write_json(evidence_dir / "revocation-negative.json", revocation_payload)
        preflight_path = evidence_dir / "preflight.json"
        if not preflight_path.exists():
            _write_json(preflight_path, preflight)
        print(f"revocation evidence: {evidence_dir / 'revocation-negative.json'}")
        return 0
    except (ActivationError, ValueError, OSError) as exc:
        code = exc.code if isinstance(exc, ActivationError) else "activation_revocation_local_error"
        print(f"revocation evidence blocked: {code}", file=sys.stderr)
        return 2
